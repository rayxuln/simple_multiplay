...
void *
renet_range_coder_create (void)
{
    ENetRangeCoder * rangeCoder = (ENetRangeCoder *) renet_malloc (sizeof (ENetRangeCoder));
    if (rangeCoder == NULL)
      return NULL;
...
void
renet_range_coder_destroy (void * context)
{
    ENetRangeCoder * rangeCoder = (ENetRangeCoder *) context;
    if (rangeCoder == NULL)
      return;

    renet_free (rangeCoder);
}
...
static enet_uint16
renet_symbol_rescale (ENetSymbol * symbol)
{
    enet_uint16 total = 0;
    for (;;)
    {
        ...
        if (symbol -> left)
          symbol -> under += renet_symbol_rescale (symbol + symbol -> left);
        total += symbol -> under;
...
{ \
    (context) -> total = (context) -> symbols ? renet_symbol_rescale ((context) + (context) -> symbols) : 0; \
...
size_t
renet_range_coder_compress (void * context, const ENetBuffer * inBuffers, size_t inBufferCount, size_t inLimit, enet_uint8 * outData, size_t outLimit)
{
    ENetRangeCoder * rangeCoder = (ENetRangeCoder *) context;
...
size_t
renet_range_coder_decompress (void * context, const enet_uint8 * inData, size_t inLimit, enet_uint8 * outData, size_t outLimit)
{
    ENetRangeCoder * rangeCoder = (ENetRangeCoder *) context;
    enet_uint8 * outStart = outData, * outEnd = & outData [outLimit];
...
*/
int
renet_host_compress_with_range_coder (ENetHost * host)
{
    ENetCompressor compressor;
    memset (& compressor, 0, sizeof (compressor));
    compressor.context = renet_range_coder_create();
    if (compressor.context == NULL)
      return -1;
    compressor.compress = renet_range_coder_compress;
    compressor.decompress = renet_range_coder_decompress;
    compressor.destroy = renet_range_coder_destroy;
    renet_host_compress (host, & compressor);
    return 0;
}
